...
class User_Login:
    ...
    def add_product(self):
        if self.product_id.get()=="" or self.product_name.get()=="":
            ...
            cur=con.cursor()
            cur.execute("insert into inventory values(%s,%s,%s,%s)",(self.product_id.get(),self.product_name.get(),self.product_qty.get(),self.total))
            con.commit()
            self.fetch_data()
            self.clear()
            con.close()
    ...
    def seller(self,id):
        ...
        self.txt_price.grid(row=4,column=1,pady=10,padx=20,sticky="w")
        ...
        self.invent_table.column("PRODUCT ID",width=75)
        self.invent_table.column("PRODUCT NAME",width=225)
        self.invent_table.column("QUANTITY",width=175)
        self.invent_table.column("PRICE",width=175)
            ...
            try:
                ...
                for (mail_id,password) in cur:
                    if self.login_iden.get()==mail_id and self.login_passen.get()==password:
                        self.seller(self.login_iden.get())
                    else:
