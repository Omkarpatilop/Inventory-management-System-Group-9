...
class Customer:
    def __init__(self,root):
        ...
        title=Label(self.root,text="INVENTORY MANAGEMENT",bd=10,relief=SOLID,fg="black",bg="cyan",font=("times new roman",40,"bold"))
        ...
        self.manage_frame=Frame(self.root,bd=5,relief=RIDGE,bg="ivory")
        ...
        #----------DETAILS FRAME----------
        self.details_frame=Frame(self.root,bd=5,relief=RIDGE,bg="ivory")
        ...
        self.invent_table=ttk.Treeview(self.table_frame,columns=("PRODUCT ID","PRODUCT NAME","QUANTITY"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.invent_table.column("PRODUCT ID",width=100)
        self.invent_table.column("PRODUCT NAME",width=250)
        self.invent_table.column("QUANTITY",width=100)
    ...
    def add_product(self):
        if self.product_id.get()=="" or self.product_name.get()=="":
            ...
            con=pymysql.connect(host="localhost",user="root",password="",database="php")
            cur=con.cursor()
            cur.execute("insert into inventory values(%s,%s,%s)",(self.product_id.get(),self.product_name.get(),self.product_qty.get()))
            con.commit()
            self.fetch_data()
            self.clear()
            con.close()
            messagebox.showinfo("success","record has been inserte")
    ...
    def search_data(self): 
        ...
        if f'{self.search_by.get()}'=='product_id':
            statement=f"select * from inventory where product_id='{self.search_txt.get()}'"
        else:
            statement=f"select * from inventory where product_name='{self.search_txt.get()}'"
